...
// ambil objek data
$data = new readPendaftaranKp ();
// ambil data pada tabel pendaftar kp
$dataRead = $data -> ambilPendaftar("SELECT * FROM mahasiswa");


//jika user mencari dat
 if (isset($_POST["cari"])) {
  $dataRead = search($_POST["keyword"],'mahasiswa','Id','Tempat_KP','Alamat_KP'); 
        ...
        <ul id="components-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
          ...
          <li>
            <a href="MasukanNilai.php">
              <i class="bi bi-circle"></i><span>Mengungah Nilai</span>
            </a>
          </li>
        </ul>
                  ...
                  <div class="search-bar">
                    <form class="search-form d-flex align-items-center" method="POST" action="#">
                      ...
                      <button type="submit" title="Search" name="cari"><i class="bi bi-search"></i></button>
                            ...
                            <table  class="table  table-bordered  border-dark table-hover shadow p-3 mb-5 bg-white rounded">
                            <thead class="table-success border-dark table-hover shadow ">                            
                            <tr>
                              <th  scope="col">No</th>
                              ...
                              <th  scope="col">Pembimbing Id</th>
                              <th  scope="col">Perusahaan Id</th>
                            </tr>
                            </thead>

                           <!-- kita akan me looping array dan dimaksukan setiap array didalma -->
                              <?php $nomor = 1 ?>
                              <?php foreach ($dataRead as $Kp ) : ?>

                                <tr>
                                  <td scope="row"><?= $nomor; ?></td>
                                  <td>
                                    <a href="ubahPendaftaranKp.php?id=<?= $Kp["Id"]?>"><img src="assets/img/editing.png" style="width:20px;"></a>|
                                    <?php //<-- onclik disini untuk konfirmasi jika datanya mau dihapus --> ?>
                                    
                                    <a href="deletePendaftaranKp.php?id= <?= $Kp["Id"];?>" onclick= " return confirm ('apakah anda yakin menghapus data ini?');"><img src="assets/img/remove.png" style="width:20px;"></a>
                                  </td>
                                  <td><?= $Kp["Id"]; ?></td>
                                  <td><?= $Kp["Tempat_KP"]; ?></td>
                                  <td><?= $Kp["Alamat_KP"];?></td>
                                  <td><?= $Kp["Tanggal_Mulai"];?></td>
                                  <td><?= $Kp["Tanggal_Selesai"]; ?></td>
                                  <td><a href="viewPendafKp.php?name=<?=$Kp["Proposal"];?>"><?=$Kp["Proposal"]; ?></a></td>
                                  <td><?=$Kp["anggota_Kelompok_Id"]; ?></td>
                                  <td><?=$Kp["Dosen_Id"]; ?></td>
                                  <td><?=$Kp["Perusahaan_Id"]; ?></td>
